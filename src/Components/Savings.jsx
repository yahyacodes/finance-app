...
const Savings = () => {
  ...
  useEffect(() => {
    const getSavings = async () => {
      const data = await getDocs(savingsCollectionRef);
      setSavings(data.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
    };
  ...
  return (
    <section className="py-14">
      ...
      <h1 className="text-3xl font-bold text-gray-100 mx-auto">Savings</h1>
            ...
            {savings.map((data) => (
              <tr key={data.id} className="odd:bg-divBg">
                ...
                <td className="text-right px-6 whitespace-nowrap">
                  <button
                    className="py-2 leading-none px-3 font-medium text-red-600 hover:text-red-500 cursor-pointer"
                    onClick={() => {
